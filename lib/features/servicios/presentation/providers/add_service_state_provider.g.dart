// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'add_service_state_provider.dart';

// **************************************************************************
// RiverpodGenerator
// **************************************************************************

String _$serviceTypeFireStoreDatasourceHash() =>
    r'73467ed63c2086a5c9152b95f4ccc9186c234078';

/// See also [serviceTypeFireStoreDatasource].
@ProviderFor(serviceTypeFireStoreDatasource)
final serviceTypeFireStoreDatasourceProvider =
    AutoDisposeProvider<ServiceTypeFireStoreDatasource>.internal(
  serviceTypeFireStoreDatasource,
  name: r'serviceTypeFireStoreDatasourceProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$serviceTypeFireStoreDatasourceHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef ServiceTypeFireStoreDatasourceRef
    = AutoDisposeProviderRef<ServiceTypeFireStoreDatasource>;
String _$serviceTypeRepositoryHash() =>
    r'2d31c33e5045eb4ce4612a34d62ca5253810b83f';

/// See also [serviceTypeRepository].
@ProviderFor(serviceTypeRepository)
final serviceTypeRepositoryProvider =
    AutoDisposeProvider<ServiceTypeFirebaseRepositoryImpl>.internal(
  serviceTypeRepository,
  name: r'serviceTypeRepositoryProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$serviceTypeRepositoryHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef ServiceTypeRepositoryRef
    = AutoDisposeProviderRef<ServiceTypeFirebaseRepositoryImpl>;
String _$addServiceTypeHash() => r'db5faa7b2340ae362c81600eda52ae289f181ac0';

/// See also [addServiceType].
@ProviderFor(addServiceType)
final addServiceTypeProvider = AutoDisposeProvider<AddServiceType>.internal(
  addServiceType,
  name: r'addServiceTypeProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$addServiceTypeHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef AddServiceTypeRef = AutoDisposeProviderRef<AddServiceType>;
String _$getServiceTypeHash() => r'a1f9f563ef11f4933295a9ac1329cf7ddd763bef';

/// See also [getServiceType].
@ProviderFor(getServiceType)
final getServiceTypeProvider = AutoDisposeProvider<GetServiceType>.internal(
  getServiceType,
  name: r'getServiceTypeProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$getServiceTypeHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef GetServiceTypeRef = AutoDisposeProviderRef<GetServiceType>;
String _$deleteServiceTypeHash() => r'1c9447ce0f5ce7f98c88b9317e70967b7c526ff5';

/// See also [deleteServiceType].
@ProviderFor(deleteServiceType)
final deleteServiceTypeProvider =
    AutoDisposeProvider<DeleteServiceType>.internal(
  deleteServiceType,
  name: r'deleteServiceTypeProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$deleteServiceTypeHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef DeleteServiceTypeRef = AutoDisposeProviderRef<DeleteServiceType>;
String _$modifieServiceTypeHash() =>
    r'5c3351f8008f932673a1e7e9e1213d52adc87a9a';

/// See also [modifieServiceType].
@ProviderFor(modifieServiceType)
final modifieServiceTypeProvider =
    AutoDisposeProvider<ModifieServiceType>.internal(
  modifieServiceType,
  name: r'modifieServiceTypeProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$modifieServiceTypeHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef ModifieServiceTypeRef = AutoDisposeProviderRef<ModifieServiceType>;
String _$typosDeVeiculosHash() => r'88e5eb783584cc3b629be6f287da1f5b3a65bcf8';

/// See also [TyposDeVeiculos].
@ProviderFor(TyposDeVeiculos)
final typosDeVeiculosProvider = AutoDisposeProvider<List<String>>.internal(
  TyposDeVeiculos,
  name: r'typosDeVeiculosProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$typosDeVeiculosHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef TyposDeVeiculosRef = AutoDisposeProviderRef<List<String>>;
String _$serviceTypeListHash() => r'7611c457be37a56cded5716fbb63369c290ec3e1';

/// See also [ServiceTypeList].
@ProviderFor(ServiceTypeList)
final serviceTypeListProvider = AutoDisposeAsyncNotifierProvider<
    ServiceTypeList, List<ServiceType>?>.internal(
  ServiceTypeList.new,
  name: r'serviceTypeListProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$serviceTypeListHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$ServiceTypeList = AutoDisposeAsyncNotifier<List<ServiceType>?>;
String _$vehiculoStateHash() => r'05e82d24d0059fd287e8236e1ba58a000592d645';

/// See also [VehiculoState].
@ProviderFor(VehiculoState)
final vehiculoStateProvider =
    AutoDisposeNotifierProvider<VehiculoState, Vehicle>.internal(
  VehiculoState.new,
  name: r'vehiculoStateProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$vehiculoStateHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$VehiculoState = AutoDisposeNotifier<Vehicle>;
String _$placaHash() => r'f84eaaed1b7c8d30a573ac8d1d1d4a7d5e350c25';

/// See also [Placa].
@ProviderFor(Placa)
final placaProvider = AutoDisposeNotifierProvider<Placa, String>.internal(
  Placa.new,
  name: r'placaProvider',
  debugGetCreateSourceHash:
      const bool.fromEnvironment('dart.vm.product') ? null : _$placaHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$Placa = AutoDisposeNotifier<String>;
String _$propietarioHash() => r'5c6bdc4d963330f258b31ef9b66781f796a07d9f';

/// See also [Propietario].
@ProviderFor(Propietario)
final propietarioProvider =
    AutoDisposeNotifierProvider<Propietario, User>.internal(
  Propietario.new,
  name: r'propietarioProvider',
  debugGetCreateSourceHash:
      const bool.fromEnvironment('dart.vm.product') ? null : _$propietarioHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$Propietario = AutoDisposeNotifier<User>;
String _$typoDeVehiculoHash() => r'8b5802deb4581c8eaa64304af6ef894cc43723b6';

/// See also [TypoDeVehiculo].
@ProviderFor(TypoDeVehiculo)
final typoDeVehiculoProvider =
    AutoDisposeNotifierProvider<TypoDeVehiculo, String>.internal(
  TypoDeVehiculo.new,
  name: r'typoDeVehiculoProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$typoDeVehiculoHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$TypoDeVehiculo = AutoDisposeNotifier<String>;
String _$isLoadingHash() => r'c455bc001c1255e8b4bdd58b41cf651a75ca4967';

/// See also [IsLoading].
@ProviderFor(IsLoading)
final isLoadingProvider = AutoDisposeNotifierProvider<IsLoading, bool>.internal(
  IsLoading.new,
  name: r'isLoadingProvider',
  debugGetCreateSourceHash:
      const bool.fromEnvironment('dart.vm.product') ? null : _$isLoadingHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$IsLoading = AutoDisposeNotifier<bool>;
String _$listPropietariosHash() => r'ed58a7162265c283eaeea6d5083babcdb57e7ad8';

/// See also [ListPropietarios].
@ProviderFor(ListPropietarios)
final listPropietariosProvider =
    AutoDisposeNotifierProvider<ListPropietarios, List<User>>.internal(
  ListPropietarios.new,
  name: r'listPropietariosProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$listPropietariosHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef _$ListPropietarios = AutoDisposeNotifier<List<User>>;
// ignore_for_file: type=lint
// ignore_for_file: subtype_of_sealed_class, invalid_use_of_internal_member, invalid_use_of_visible_for_testing_member
